...
from MACT_utils import get_flux_from_FAST as get_flux
...
def get_lnu(filt_index):
    ...
    '''
    ID    = corrID[filt_index]
    zspec = corrzspec0[filt_index]
...
def get_corr12(filt_match):
    ...
    filtnames = corrzspec0[filt_match]
...
def plot_just_balmer_decrements():
    ...
    for (ff, cc) in zip(filt_arr, color_arr):
        ...
        filtz = corrzspec0[filt_match][index_match]
...
corrNAME0 = np.delete(corrNAME0, bad_index)
...
corrzspec0 = np.delete(corrzspec0, bad_index)
corrID = np.delete(corrID, bad_index)
